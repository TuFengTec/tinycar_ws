...
class mapOptimization : public ParamServer {
 public:
  ...
  mapOptimization() {
    ...
    Tic = Eigen::Matrix4f::Identity();
    Eigen::Vector3f tic(extGpsTrans[0], extGpsTrans[1], extGpsTrans[2]);

    std::cout<<"tictictictictictic="<<tic<<std::endl;
  ...
  void gpsHandler(const nav_msgs::Odometry::ConstPtr &gpsMsg) {
    //  auto G_msg = Odom_A2B(gpsMsg,Tic);
    if (useGPS) {
  ...
  bool syncGPS(std::deque<nav_msgs::Odometry> &gpsBuf,
    ...
    while (!gpsQueue.empty()) {
      ...
      if (gpsQueue.front().header.stamp.toSec() < timestamp - eps_cam) {
        ...
        if (debugGps)
                 ROS_INFO("message too old, gps=%f ,cloud=%f",
                          gpsQueue.front().header.stamp.toSec(),timestamp);
        // message too old
        gpsQueue.pop_front();
        mtxGpsInfo.unlock();
      } else if (gpsQueue.front().header.stamp.toSec() > timestamp + eps_cam) {
         if (debugGps)
                 ROS_INFO("message too New!,  gps=%f ,cloud=%f",
                          gpsQueue.front().header.stamp.toSec(),timestamp);
        // message too new
        mtxGpsInfo.unlock();
        break;
        ...
        gpsQueue.pop_front();
               if (debugGps)
                 ROS_INFO("GPS time offset %f ,gps=%f ,cloud=%f",
                          aligedGps.header.stamp.toSec() - timestamp,
                          gpsQueue.front().header.stamp.toSec(),
                          timestamp);
        mtxGpsInfo.unlock();
        break;
      }
    }
    ROS_INFO("hasGps= %d ",hasGPS);
    if (hasGPS)
      return true;
    else
      return false;
...
//                                              &cloudKeyPoses6D->points[i]);
  ...
  void updateInitialGuess() {
    ...
    if (cloudKeyPoses3D->points.empty()) {
      ...
      if (useGPS) {
        ...
        if (syncGPS(gpsQueue, alignedGPS, timeLaserInfoCur,
          ...
          // auto alignedGPS = Odom_A2B(tempGPS,Tic);
          ...
          if (debugGps) {
            ...
            std::cout << "initial gps yaw_t: " << yaw_t *180/M_PI << std::endl;
          ...
          transformTobeMapped[0] = roll_t;
          transformTobeMapped[1] = pitch_t;
          transformTobeMapped[2] = yaw_t;

          std::cout << "transformTobeMapped[3,4,5]: " << transformTobeMapped[3]
    ...
    {
      Eigen::Affine3f transBack = pcl::getTransformation(0, 0, 0, cloudInfo.imuRollInit,
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  void addGPSFactor() {
    ...
    if (syncGPS(gpsQueue, thisGPS, timeLaserInfoCur, 1.0 / gpsFrequence)) {
      ...
      if (abs(noise_x) > gpsCovThreshold || abs(noise_y) > gpsCovThreshold || (thisGPS.pose.covariance[4]<5))
        return;
      ...
      gtsam::Vector Vector3(3);
      // Vector3 << noise_x, noise_y, noise_z;
      Vector3 << max(noise_x, 0.2f), max(noise_y, 0.2f), max(noise_z, 0.3f);
      noiseModel::Diagonal::shared_ptr gps_noise =
          noiseModel::Diagonal::Variances(Vector3);
