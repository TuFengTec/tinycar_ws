...
//              << ", " << imuRotZ[imuPointerCur - 1] << std::endl;
    ...
    void projectPointCloud() {
        ...
        { //xconvert cloud to GPS/base_link frame
            pcl::PointCloud<PointXYZIRT>::Ptr transformed_cloud (new pcl::PointCloud<PointXYZIRT> ());
            Eigen::Matrix4f transform_1 = Eigen::Matrix4f::Identity();
            // transform_1.block<3, 3>(0, 0) = qua.normalized().toRotationMatrix();
            // Eigen::Vector3f tran(-0.253114, 0.0275, -0.75135);

            Eigen::Vector3f tran(extGpsTrans[0], extGpsTrans[1], extGpsTrans[2]);
            transform_1.block<3, 1>(0, 3) = tran;

            pcl::transformPointCloud (*laserCloudIn, *transformed_cloud, transform_1);
            laserCloudIn.swap(transformed_cloud);
        }
