...
public:
    ...
    vector<double> extGpsTransV;  //GPS
    Eigen::Matrix3d extRot;
    Eigen::Matrix3d extRPY;
    Eigen::Vector3d extTrans;
    Eigen::Vector3d extGpsTrans;//GPS
    ...
    ParamServer() {
        ...
        nh.param<vector<double >>("lio_sam_6axis/extrinsicGpsTrans", extGpsTransV, vector<double>());
        
        extRot = Eigen::Map<const Eigen::Matrix<double, -1, -1, Eigen::RowMajor>>(extRotV.data(), 3, 3);
        extRPY = Eigen::Map<const Eigen::Matrix<double, -1, -1, Eigen::RowMajor>>(extRPYV.data(), 3, 3);
        extTrans = Eigen::Map<const Eigen::Matrix<double, -1, -1, Eigen::RowMajor>>(extTransV.data(), 3, 1);
        extGpsTrans = Eigen::Map<const Eigen::Matrix<double, -1, -1, Eigen::RowMajor>>(extGpsTransV.data(), 3, 1); //gps
...
///////////////////////////////////////////////////////////////////////////////
Eigen::Matrix4f  Frame_A2B(
    /* [in] */ const Eigen::Matrix4f &pose,
    /* [in] */ const Eigen::Matrix4f &Tic ) {
  return (pose * Tic);
}

nav_msgs::Odometry Odom_A2B(const nav_msgs::Odometry &odomFrom, const Eigen::Matrix4f &Tic)
{
 
    // Eigen::Matrix4f Tic = Eigen::Matrix4f::Identity();
    // Eigen::Vector3f tic(-l2b_x_, -l2b_y_, -l2b_z_);
    // Tic.block<3, 1>(0, 3) = tic;

    Eigen::Matrix4f pose_from = Eigen::Matrix4f::Identity();
    Eigen::Quaternionf qua(odomFrom.pose.pose.orientation.x,
                           odomFrom.pose.pose.orientation.y,
                           odomFrom.pose.pose.orientation.z, 
                           odomFrom.pose.pose.orientation.w);
    Eigen::Vector3f t(odomFrom.pose.pose.position.x,
                      odomFrom.pose.pose.position.y,
                      odomFrom.pose.pose.position.z);
    // Eigen::Vector3f t(0,
    //                   0,
    //                   0);
    pose_from.block<3, 3>(0, 0) = qua.normalized().toRotationMatrix();
    pose_from.block<3, 1>(0, 3) = t;

    Eigen::Matrix4f pose_to = Frame_A2B(pose_from, Tic);
    

    Eigen::Quaternionf qua_to(pose_to.block<3, 3>(0, 0));
    Eigen::Vector3f t_to = pose_to.block<3, 1>(0, 3);

    std::cout<<"t_to ="<<t_to<<std::endl;
 
    nav_msgs::Odometry odomTo;
    odomTo = odomFrom;
    odomTo.pose.pose.orientation.x = qua_to.x();
    odomTo.pose.pose.orientation.y = qua_to.y();
    odomTo.pose.pose.orientation.z = qua_to.z();
    odomTo.pose.pose.orientation.w = qua_to.w();

    odomTo.pose.pose.position.x = t_to[0];
    odomTo.pose.pose.position.y = t_to[1];
    odomTo.pose.pose.position.z = t_to[2];
    return odomTo;

}

// north yaw=0 ============> easth yaw = 0;
double conver_frame(const double & yaw_from)
{
    double yaw_to = yaw_from + M_PI/2;
    if(yaw_to>M_PI*2)
       yaw_to =  yaw_to-M_PI*2 ;
